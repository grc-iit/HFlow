...
template<class T>
Rhea::Daemon<T>::Daemon(int argc, char *argv[], std::string main_log_file): main_log_file(main_log_file), jobManager() {
...
template<class T>
Rhea::Daemon<T>::~Daemon() {
    exitSignal.set_value();
    worker.join();
}

template<class T>
void Rhea::Daemon<T>::signalHandler(int sig) {
    switch (sig) {
        case SIGHUP: {
...
template<class T>
void Rhea::Daemon<T>::logMessage(const std::string &filename, const std::string &message) {
...
template<class T>
void Rhea::Daemon<T>::catchSignals() {
